...
const snapshotPath = path.join(__dirname, 'tree-snapshot.json');
fs.writeFileSync(snapshotPath, JSON.stringify({ roots: rootNodes.map(r=>r.id), augustine: { id: augustine.id, children: augustine.children.map(c=>c.id) }, familyUnits: fus.map(f=> ({ id: f.id, children: (f.children||[]).map(c=>c.id) })) }, null, 2));
